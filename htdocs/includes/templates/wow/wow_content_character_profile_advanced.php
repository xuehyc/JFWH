    ...
    foreach($item_slots as $slot => $data) {
        ...
        for($i = 0; $i < 3; ++$i) {
            if(is_array($item_info['g' . $i]) && isset($item_info['g' . $i]['item'])) {
                $socket_text .= sprintf('<span class="icon-socket socket-%d"><a href="%s/wow/' . WoW_Locale::GetLocale() . '/item/%d" class="gem"><img src="http://www.wowdb.tw/icons/middle/%s.jpg" alt="" /><span class="frame"></span></a></span>', $item_info['g' . $i]['color'], WoW::GetWoWPath(), $item_info['g' . $i]['item'], $item_info['g' . $i]['icon']);
            }
        }
        echo sprintf('<div data-id="%d" data-type="%d" class="%s" style="%s">
        <div class="slot-inner">
            <div class="slot-contents">
                <a href="%s/wow/' . WoW_Locale::GetLocale() . '/item/%d" class="item" data-item="%s"><img src="http://www.wowdb.tw/icons/middle/%s.jpg" alt="" /><span class="frame"></span></a>
                <div class="details">
	...
	//]]>
            ...
            ?>
                ...
                if(isset($audit[AUDIT_TYPE_USED_GEMS]) && is_array($audit[AUDIT_TYPE_USED_GEMS])) {
                    ...
                    <img src="http://www.wowdb.tw/icons/middle/%s.jpg" alt="" />
